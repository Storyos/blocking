...
const Notification = () => {
  ...
  const [notifications, setNotifications] = useState([
    {
      text: "새로운 알림이 있습니다. 이 알림은 매우 긴 텍스트를 포함하고 있으며 전체 내용을 보려면 아래 화살표 아이콘을 클릭하세요.",
      read: false,
      expanded: false,
    },
    {
      text: "계정 설정이 업데이트되었습니다. 계정 설정 업데이트 내용은 다음과 같습니다: 비밀번호 변경, 이메일 주소 업데이트, 전화번호 추가.",
      read: false,
      expanded: false,
    },
    {
      text: "비밀번호가 성공적으로 변경되었습니다. 비밀번호 변경 내역은 보안 설정에서 확인할 수 있습니다.",
      read: false,
      expanded: false,
    },
  ]);
  ...
  const toggleExpand = (index) => {
    setNotifications((prevState) =>
      prevState.map((notification, i) =>
        i === index ? { ...notification, expanded: !notification.expanded, read: true } : notification
      )
    );
  };
        ...
        {notifications.map((notification, index) => (
          ...
          >
            <IconContainer>
              ...
              <NotificationText expanded={notification.expanded}>{notification.text}</NotificationText>
